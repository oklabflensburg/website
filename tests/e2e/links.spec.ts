...
for (const locale of locales) {
  test(`${locale}: external links are safe new-tab links and internal links stay same-tab`, async ({ page }) => {
    ...
    const source = page.locator('.detail-aside').getByRole('link', { name: messages[locale].common.source })
    await expect(source).toHaveAttribute('target', '_blank')
    await expect(source).toHaveAttribute('rel', 'noopener noreferrer')
    const profile = page.locator('footer').getByRole('link', { name: 'Mastodon', exact: true })
    await expect(profile).toHaveAttribute('href', site.social.mastodon)
    await expect(profile).toHaveAttribute('target', '_blank')
    await expect(profile).toHaveAttribute('rel', 'me noopener noreferrer')
