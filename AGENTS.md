...
- Content is the only editorial source of truth: no Vue project arrays, JSON mappings, project descriptions in UI dictionaries or second blog engine. Query project/blog details by `locale` and localized `slug`, and editorial pages by `locale` and `translationKey`; join translations by collection and stable `translationKey`.
- Use the same `translationKey` for translations of one resource; slugs may differ. Keys, slugs and optional previous-slug `aliases` use lowercase ASCII words separated by hyphens. Missing translations are legitimate; never invent alternates. Project/blog/page descriptions and project image descriptions must exceed 20 characters to satisfy the current content tests.
...
- German uses `/`, Danish `/da`, English `/en`, with localized segments from `routePaths` in the site config. Use `useLocalePath` in Vue; `LanguageSwitcher` and [ProseA](app/components/content/ProseA.vue) resolve actual content translations through the derived translation index. The shared `localizedPath` helper is for server/pure utilities. Preserve detail-page language switching and the cookie-free locale strategy. See [localized-routing.md](docs/localized-routing.md).

### Add or update a project

1. Verify its purpose, source, links and status. Create `content/{de,da,en}/projects/<translationKey>.md` for available locales; follow [open-city-planner.md](content/de/projects/open-city-planner.md).
2. Required frontmatter: `locale`, `slug`, `translationKey`, `title`, `description`, `status`, nonempty `categories`, `technologies` (array), `image`, `imageAlt`, `links` (object) and `source` (URL). `links.website` and `links.github` are optional URLs. `featured` defaults to false; `featuredOrder` defaults to 0 and must be a nonnegative integer.
...
4. Add one distinct, text-free signet at `public/images/projects/<translationKey>.svg`; set `image` to `/images/projects/<translationKey>.svg` and write a descriptive `imageAlt` in each language. Follow the 1024-square geometry, safe SVG and palette rules in [project-logo-system.md](docs/project-logo-system.md). No photos, screenshots, embedded text or generic organisation logo as a project signet. Display the complete image with `object-contain`.
   The user-supplied kulturbytes artwork is an approved exception to the text-free signet/palette rules: preserve its pink symbol and outlined wordmark within the same square format (see the logo-system guide).
5. Write the Markdown body, run `pnpm brand:generate`, and review/commit the derived project social card with the Content and SVG. The generator reads all existing locale Content records and deduplicates by translationKey; do not add a separate project list.

### Blog, pages, events and team

- Blog: `content/{de,da,en}/blog/<slug>.md`; required `locale`, `slug`, `translationKey`, `title`, `description`, quoted `date: 'YYYY-MM-DD'`, nonempty `authors`, and `tags` (array). Optional `updated` date and local `image`; set `draft: true` explicitly while preparing a post (also the schema default). Publish only after editorial confirmation. Draft/future posts must stay out of detail routes, listings, RSS and sitemap. All blog social previews use the branded default; an optional article image is content imagery.
- Editorial pages: `content/{de,da,en}/pages/<translationKey>.md`, with `locale`, `translationKey`, `title`, `description` and optional `noindex` (default false). Pages have no content `slug`; filenames do not define public URLs. Each editorial route entry renders the single `app/components/EditorialPage.vue`; Nuxt i18n gets its paths and stable editorial key from the site config through `nuxt.config.ts`. Update the route entry, `editorialSlugs`, `routePaths` and locale navigation keys together when adding a route. Unknown paths must remain 404s.
- General contact UI and the Lab’s schema.org `Organization` use `site.contact.email`. Legal/operator email belongs only to `runtimeConfig.public.legal` via `useLegalContact()`; the two inboxes may differ and must not fall back to each other.
- Legal pages embed `app/components/content/LegalDetails.vue` through MDC; operator and hosting identities plus the explicit `hostingDpa` boolean come only from `runtimeConfig.public.legal` via `useLegalContact()`. The `kind="hosting"` variant receives translated prose through Content slots; its `#dpa` slot renders only when the boolean is true. Never copy deployment identity into Markdown or `site.ts`. Preserve the production startup gate and `noindex`; see [legal-configuration.md](docs/legal-configuration.md) for environment variables, fictional test data and publication review.
- One-off events: `content/{de,da,en}/events/<slug>.yml`; required `locale`, `slug`, `translationKey`, `title`, `description`, `kind`, `start`, `end`, `location`, `address`, `url`, `status`; optional `registrationUrl`. Use schema enums and ISO timestamps with explicit timezone offsets; retain cancellation/postponement and past-event information. The regular meeting comes only from `site.meeting` and `shared/utils/meeting.ts`, not duplicated event files or editorial addresses/schedules.
...
The optional homepage Mastodon feed is server-fetched through `/api/social/mastodon`, with process-local caching and safe plain-text DTOs. Reuse `site.social.mastodon`; never add browser fetches, remote media or raw upstream HTML. Preserve content warnings and outage fallback. See [mastodon.md](docs/mastodon.md) for configuration, cache behavior and deterministic test fixtures.
...
- Keep images/fonts local and retain provenance/licences. Prefer `NuxtImg` with explicit dimensions and meaningful alt text; existing direct SVG project images remain valid. Use existing assets; the approved Plausible integration in `app/app.vue` is the sole analytics integration (see [analytics.md](docs/analytics.md)). Do not add other trackers or unnecessary third-party embeds.
...
- Use `usePageSeo` for localized titles/descriptions, OpenGraph/Twitter and WebPage data. `usePageSeo` also owns canonical and actual Content-based hreflang; sitemap uses the same translation groups. `app/app.vue` uses `useLocaleHead({ seo: false })` only for language attributes, plus Organization/WebSite data. The public translation index is derived from existing Content collections, with draft/future/noindex filtering and no invented language variants or x-default. Avoid competing head implementations.
...
- Render ordinary Vue anchors through `SiteLink.vue`; use `NuxtLink` for localized application navigation. `shared/utils/links.ts` is the sole external-link detector, comparing HTTP(S) origins with `site.url`. External links always receive `target="_blank"` and `rel="noopener noreferrer"` (preserve meaningful tokens such as `me`); internal/canonical absolute links, locale switching, breadcrumbs, fragments, mail and phone links stay in the same tab. `ProseA.vue` applies the same rule centrally to Content and preserves translation-aware routing, queries and fragments. Keep the localized accessible new-tab description and link regression tests.
