...
describe('rendered link components', () => {
  ...
  it.each(['/projekte/karte?q=a%20b#map', site.url + '/projekte/karte?q=a%20b#map'])('preserves semantic rel values on internal Markdown link %s', async (href) => {
    const rel = 'author bookmark nofollow'
    const { attrs } = await render(ProseA, { href, rel, target: '_blank' })
    expect(attrs).toMatchObject({ href: '/en/projects/map?q=a%20b#map', rel, 'data-nuxt-link': 'true' })
    expect(attrs.target).toBeUndefined()
  })
