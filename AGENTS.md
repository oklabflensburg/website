...
- Legal pages embed `app/components/content/LegalDetails.vue` through MDC; operator identity comes only from `runtimeConfig.public.legal` via `useLegalContact()`. Never copy deployment identity into Markdown or `site.ts`. Preserve the production startup gate and `noindex`; see [legal-configuration.md](docs/legal-configuration.md) for environment variables, fictional test data and publication review.
