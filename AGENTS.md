...
- Content is the only editorial source of truth: no Vue project arrays, JSON mappings, project descriptions in UI dictionaries or second blog engine. Query details by `locale` and localized `slug`; join translations by collection and stable `translationKey`.
...
- Editorial pages: `content/{de,da,en}/pages/<slug>.md`, with `locale`, `slug`, `translationKey`, `title`, `description` and optional `noindex` (default false). Each editorial route entry renders the single `app/components/EditorialPage.vue`; Nuxt i18n gets its paths and stable editorial key from the site config through `nuxt.config.ts`. Update the route entry, `editorialSlugs`, `routePaths` and locale navigation keys together when adding a route. Unknown paths must remain 404s.
